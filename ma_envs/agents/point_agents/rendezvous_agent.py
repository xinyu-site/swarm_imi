...
class PointAgent(Agent):
    ...
    def get_observation(self, dm, my_orientation, their_orientation, vels, nh_size,agent_id):
    # w_vel 是0，以自己为坐标系，求相对速度
        if self.obs_mode == 'fix_acc':
            ...
            x=np.concatenate((dm.reshape([self.n_agents,1]),my_orientation.reshape([self.n_agents,1]),their_orientation.reshape([self.n_agents,1]),vels),axis=1)
